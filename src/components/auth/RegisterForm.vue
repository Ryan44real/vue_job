...
import { useRegisterService, sendMailCon } from '@/api/user'
...
const rePasswordValid = (_rule: unknown, value: string, callback: (err?: Error) => void) => {
  if (value === '') {
...
async function handleRegister() {
  ...
  loading.value = true
  try {
    if (form.value.password === form.value.repassword && form.value.username !== '') {
      const result: any = await useRegisterService(form.value)
...
function remove() {
  form.value = { username: '', password: '', repassword: '', mail: '', code: '' }
}
</script>
...
<template>
  <div class="glass-form">
    <div class="glass-form__header">
      <h2 class="glass-form__greeting">创建账户</h2>
      <p class="glass-form__subtitle">注册以开始使用图书管理系统</p>
    </div>
    ...
    >
      ...
      <p class="glass-form__footer">
        已有账户？
        <el-button link type="primary" class="glass-link" @click="remove(); emit('switch', 'login')">
          返回登录
        </el-button>
      </p>
    </el-form>
