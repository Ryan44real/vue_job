...
| 旧文件 | 新文件 | 备注 |
|--------|--------|------|
| Layout.vue + MyLayout.vue | AppLayout.vue + AppSidebar + AppNavbar | 统一布局 |
| Login.vue | LoginView.vue + LoginForm + RegisterForm + ForgotPasswordForm | 模块化拆分 |
| DashShow.vue + userMain.vue | HomeView.vue | 角色自适应 |
| BorrowBooks.vue + visiter.vue | SearchView.vue | 合并+访客模式 |
| ArticleManage.vue | BookManageView.vue + BookEditDrawer | 抽屉式编辑 |
| ArticleCategory.vue | CategoryManageView.vue | 重构 |
| BorrowRecordManage.vue | BorrowManageView.vue | 重构 |
| RenewalManage.vue | RenewalManageView.vue | 重构 |
| UserManage.vue | UserManageView.vue | 重构 |
| MyborrowRecord.vue | BorrowingListView + BorrowingHistoryView + RenewView | 分拆 |
| UserInfo + UserAvatar + UserResetPassword | SettingsView.vue（Tab 页） | 合并 |
| Test.vue | BookScanner.vue | 组件化封装 |
